...
const rulesInput = trilogyRules.join("\n, ")

export function createPrompt(query: string, concepttInputs: ModelConceptInput[] = testFields) {
  const fields = concepttInputs.map(field => `name:${field.name} type:${field.type}`).join(", ")
...
const testCases: TestScenario[] = [
...

  {
    ...
    prompt: createPrompt('Who were the top ten customers by sales in august 1995 who had orders that shipped in october or september?'),
    expectedResponse: {
      contains: ["extended_price", "order.customer.name", "8", "::date"],
      mustIdentify: "order.customer.name",
      notContains: ['FROM', 'GROUP']
