export const trilogyRules = [
  ...
  'use date_add to manipulate dates, like date_add(ship_date, month, 3). Use date_diff for date differences. It expects date_diff(<date_to_subtract>, <date_to_subtract_from>, DAY|MONTH|YEAR|SECOND|HOUR|ETC) date_diff will subtract the first date from the second date, so a early and later is positive.',
  'Avoid naming fields after SQL keywords. For example, do not name a field "order" or "select". Do not use quotes on fields. Prefer underscores and avoid special characters.',
