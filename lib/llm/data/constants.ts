export const trilogyRules = [
  ...
  'You can derive a new field from an existing field using sql functions in any position, and aggregates can have inline grouping - for example, sum(revenue) by state as per_state_sales, sum(revenue) by country as country_sales would be valid selects. AWLAYS alias a calculation, and you must use the by keyword.',
