...
const ICON_PATH_MAP: Record<(typeof ICON_CLASS_NAMES)[number], string> = {
  ...
  'mdi-comment-quote-outline': mdiCommentQuoteOutline,
...
function buildIconCss(): string {
  return ICON_CLASS_NAMES.map((iconClassName) => {
    const path = ICON_PATH_MAP[iconClassName]
...
function resolveMdiIconPath(classNames: Iterable<string> | string): string | null {
  const names =
  ...
  for (const className of names) {
    if (className in ICON_PATH_MAP) {
      return ICON_PATH_MAP[className as keyof typeof ICON_PATH_MAP]
    }
  }
