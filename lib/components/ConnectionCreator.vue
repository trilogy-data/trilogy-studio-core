<template>
    <div>
      ...
      <div v-if="visible">
        <form @submit.prevent="submitConnectionCreation">
          <div>
            ...
            <input type="text" v-model="editorDetails.name" id="connection-name" required />
          </div>
  
          <div>
            <label for="connection-type">Connection Type:</label>
            <select v-model="editorDetails.type" id="connection-type" required>
              <option value="duckdb">Duckdb</option>
              <option value="motherduck">MotherDuck</option>
              <option value="bigquery ">MotherDuck</option>
            </select>
          </div>
      ...
      <div v-if="submittedConnection">
        <p>Connection created with the following details:</p>
        <p>Name: {{ submittedConnection.name }}</p>
        <p>Type: {{ submittedConnection.type }}</p>
        <p>Connection: {{ submittedConnection.connection }}</p>
      </div>
    </div>
  </template>
...
import type {ConnectionStoreType} from '../data/connections';
  ...
  button:hover {
    background-color: #6c6c6c; /* Lighter gray when hovered */
  }
  
  form {
    margin-top: 20px;
    padding: 20px;
    background-color: #e0e0e0; /* Gray background for form */
  ...
  input:focus, select:focus {
    border-color: #4b4b4b; /* Dark gray border on focus */
    outline: none;
  }
