<template>
    ...
    >
      ...
      <div class="connection-actions">
        ...
        <button 
          v-if="item.type === 'connection' && !isSelected" 
          class="set-active-btn"
          @click.stop="setAsActive(item.id)"
  ...
  export default defineComponent({
    ...
    emits: ['toggle', 'refresh', 'updateApiKey'],
    setup(props, { emit }) {
      ...
      const handleItemClick = () => {
        if (isExpandable.value) {
          emit('toggle', props.item.id, props.item.connection?.constructor.name || '', props.item.type)
        }
      }
      ...
      const handleRefreshConnectionClick = (event: Event) => {
        ...
        emit('refresh', props.item.id, props.item.connection?.constructor.name || '', 'connection')
      }
      ...
      const setAsActive = (id: string) => {
        ...
        emit('toggle', id, props.item.connection?.constructor.name || '', 'connection')
      }
      
      // Update API key
      const updateApiKey = (connection: any, apiKey: string) => {
        emit('updateApiKey', connection, apiKey)
        apiKeyInput.value = '' // Clear input after update
      }
