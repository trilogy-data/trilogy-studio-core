...
export default {
  ...
  setup() {
    ...
    const flatList = computed(() => {
      const list: Array<{ id: string; name: string; indent: number; count: number; type: string, concept: any }> = [];

      Object.values(modelStore.models).forEach((model) => {
        let modelId = ['model', model.name].join(KeySeparator);
        list.push({ id: modelId, name: model.name, indent: 0, count: model.sources.length, type: 'model', concept: null });

        if (!collapsed.value[modelId]) {
          model.sources.forEach((source) => {
            let sourceId = ['source',model.name, source.alias].join(KeySeparator)
            list.push({ id: sourceId, name: source.alias, indent: 1, count: source.concepts.length, type: 'source', concept: null });
            if (!collapsed.value[sourceId]) {

              source.concepts.forEach((concept) => {
                list.push({ id: ['concept', model.name, source.alias, concept.namespace, concept.name].join(KeySeparator), name: concept.namespace === 'local' ? concept.name : (concept.namespace + '.' + concept.name), indent: 2, count: 0, type: 'concept', concept: concept });
              });
              source.datasources.forEach((ds) => {
                let dsId = ['datasource', model.name, source.alias, ds.name].join(KeySeparator)
                list.push({
                  id: dsId
                  , name: ds.name, indent: 2, count: ds.concepts.length, type: 'datasource', concept: null
                });
                if (!collapsed.value[dsId]) {
                  ds.concepts.forEach((field) => {
                    list.push({
                      id: ['concept', model.name, source.alias, ds.name, field.namespace, field.name].join(KeySeparator)
                      , name: field.namespace === 'local' ? field.name : (field.namespace + '.' + field.name), indent: 3, count: 0, type: 'concept', concept: field
                    });
                  });
                }
              });
