    ...
    <div class="query-history">
        <div class="query-history-header">
            ...
            <div class="query-history-actions">
                <button @click="refreshHistory" class="refresh-btn" title="Refresh history">
                    🔄
                </button>
        ...
        <div v-else class="query-history-list">
            <div v-for="item in history" :key="item.id" class="query-history-item"
                ...
                <div class="query-history-item-header" @click="toggleExpand(item.id)">
                    ...
                    <div class="query-history-item-expand">
                        {{ expandedItems[item.id as number] ? '▼' : '▶' }}
                    </div>
                </div>

                <div v-if="expandedItems[item.id as number]" class="query-history-item-details">
                    ...
                    <div class="query-history-item-actions">
                        <button @click="handleSelectQuery(item.query)" class="use-query-btn">
                            Use this query
                        </button>
                    </div>
                </div>
...
export default defineComponent({
    ...
    setup(props) {
        const {
            ...
            clearHistory,
            refreshHistory
        } = useQueryHistory(props.connectionName);
        ...
        // Handle selecting a query to rerun
        const handleSelectQuery = (query: string): void => {
            if (props.onSelectQuery) {
                props.onSelectQuery(query);
            }
        };
