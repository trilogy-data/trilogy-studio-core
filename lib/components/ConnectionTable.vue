    ...
    <div class="table-viewer" >
        ...
        <div v-if="activeTab === 'structure'" class="table-structure">
            <div class="structure-header">
                ...
                <div class="column-count">{{ filteredColumns.length }} column{{ filteredColumns.length !== 1 ? 's' : ''
                    }}</div>
            </div>
                    ...
                    <tbody>
                        <tr v-for="(row, rowIndex) in selectedSampleData" :key="rowIndex">
                            <td v-for="column in table.columns" :key="column.name">
                                <div class="cell-content" :class="getCellClass(row[column.name], column.trilogyType)">
    ...
    setup(props) {
        ...
        const loadSampleData = async () => {
            ...
            try {
                ...
                const result = await connectionStore.connections[props.connectionName].getTableSample(props.database, props.table.name, 50);
        ...
        const selectedSampleData = computed(() => sampleData.value[props.table.name]? sampleData.value[props.table.name] : []);
