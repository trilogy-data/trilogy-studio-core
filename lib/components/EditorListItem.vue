<template>
    <div :data-testid="`editor-list-id-${item.key}`" :class="{
        ...
        <div v-if="!['creator'].includes(item.type) && !isMobile" v-for="_ in item.indent" class="sidebar-padding">
        </div>
        ...
        <span class="truncate-text">
            ...
            <span class="text-light" v-if="item.type === 'connection'">
                ...
                <span v-else class="no-model-text" @click.stop="createDefaultModel(item.label)"
        ...
        <tooltip v-if="item.type === 'editor'" content="Delete Editor" position="left">
            <span class="remove-btn" @click.stop="$emit('delete-editor', item.editor)"
