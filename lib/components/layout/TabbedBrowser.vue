<template>
    <div class="tabbed-container">
        <!-- Tab Navigation Bar -->
        <div class="tab-bar">
            <div v-for="(tab, index) in tabs" :key="tab.id" :class="['tab', { 'tab-active': currentTabId === tab.id }]"
                @click="selectTab(tab.id)" @dragstart="handleDragStart(index, $event)"
                @dragover="handleDragOver($event)" @drop="handleDrop(index, $event)" draggable="true">
...
export default defineComponent({
    ...
    methods: {
        ...
        addTab(screen: ScreenType, title?: string, params?: Record<string, string>): string {
            const existingTab = this.tabs.find((tab: Tab) =>
                tab.screen === screen &&
