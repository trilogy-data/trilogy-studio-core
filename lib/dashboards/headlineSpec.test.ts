...
// Use ReturnType to infer the actual return type from the function
type VegaLiteSpec = ReturnType<typeof createHeadlineSpec>
...
describe('createHeadlineSpec', (): void => {
  ...
  describe('basic functionality', (): void => {
    it('should create a valid vega-lite spec', (): void => {
      const spec: VegaLiteSpec = createHeadlineSpec(mockData, mockColumns, 'light' as Theme)
    ...
    it('should handle empty data', (): void => {
      const spec: VegaLiteSpec = createHeadlineSpec(null, mockColumns, 'light' as Theme)
    ...
    it('should handle empty columns', (): void => {
      ...
      const spec: VegaLiteSpec = createHeadlineSpec(mockData, emptyColumns, 'light' as Theme)
  ...
  describe('desktop layout (isMobile: false)', (): void => {
    it('should distribute elements horizontally', (): void => {
      const spec: VegaLiteSpec = createHeadlineSpec(mockData, mockColumns, 'light' as Theme, false)
      ...
      const layersWithFixedDy = spec.layer.filter((layer) => typeof layer.mark.dy === 'number')
    ...
    it('should use width-based font sizing for desktop', (): void => {
      ...
      const fontSizeExpressions: (string | undefined)[] = spec.layer
        .map((layer) => layer.mark.fontSize?.expr)
        .filter(Boolean)
    ...
    it('should center single metric horizontally', (): void => {
      ...
      const spec: VegaLiteSpec = createHeadlineSpec(mockData, singleColumn, 'light' as Theme, false)

      // Single metric should have dx: 0 (centered)
      spec.layer.forEach((layer): void => {
  ...
  describe('mobile layout (isMobile: true)', (): void => {
    it('should distribute elements vertically', (): void => {
      const spec: VegaLiteSpec = createHeadlineSpec(mockData, mockColumns, 'light' as Theme, true)

      // Check that dy uses height-based expressions
      const layersWithDyExpr = spec.layer.filter(
        (layer) => layer.mark.dy && typeof layer.mark.dy === 'object' && layer.mark.dy.expr,
      )
    ...
    it('should center single metric vertically', (): void => {
      ...
      const spec: VegaLiteSpec = createHeadlineSpec(mockData, singleColumn, 'light' as Theme, true)
    ...
    it('should use smaller font sizes for mobile', (): void => {
      const mobileSpec: VegaLiteSpec = createHeadlineSpec(
        mockData,
        mockColumns,
        'light' as Theme,
        true,
      )
      const desktopSpec: VegaLiteSpec = createHeadlineSpec(
        mockData,
        mockColumns,
        'light' as Theme,
        false,
      ...
      const mobileFontExpr: string = mobileSpec.layer[0].mark.fontSize?.expr || ''
      const desktopFontExpr: string = desktopSpec.layer[0].mark.fontSize?.expr || ''
  ...
  describe('theming', (): void => {
    ...
    it('should apply dark theme colors', (): void => {
      const spec: VegaLiteSpec = createHeadlineSpec(mockData, mockColumns, 'dark' as Theme)
  ...
  describe('text formatting', (): void => {
    it('should format labels with capitalized words', (): void => {
      ...
      const labelLayers = spec.layer.filter(
        (layer) => layer.encoding?.text?.value && typeof layer.encoding.text.value === 'string',
      )

      expect(labelLayers).toHaveLength(3)
      expect(labelLayers.map((l) => l.encoding.text.value)).toEqual(['Revenue', 'Profit', 'Users'])
    })
  })
  ...
  describe('responsive font scaling', (): void => {
    it('should scale font size based on number of metrics', (): void => {
      ...
      const twoMetricSpec: VegaLiteSpec = createHeadlineSpec(mockData, twoColumns, 'light' as Theme)
      const eightMetricSpec: VegaLiteSpec = createHeadlineSpec(
        mockData,
        eightColumns,
        'light' as Theme,
      )
      ...
      const twoMetricFontExpr: string = twoMetricSpec.layer[0].mark.fontSize?.expr || ''
      const fourMetricFontExpr: string = eightMetricSpec.layer[0].mark.fontSize?.expr || ''
