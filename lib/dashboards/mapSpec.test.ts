...
vi.mock('./helpers', () => ({
  getColumnHasTrait: vi.fn((fieldName: string, columns: Map<string, ResultColumn>, trait: string) => {
...
const createUSCoordinateData = (): readonly Row[] => [
  ...
  { city: 'New York', latitude: 40.7128, longitude: -74.0060, population: 8400000 },
...
describe('createMapSpec', () => {
  ...
  describe('US Choropleth Maps', () => {
    it('should generate valid US choropleth map spec', () => {
      ...
      const spec = createMapSpec(config, data, columns, false, intChart)
    ...
    it('should include interaction parameters for choropleth maps', () => {
      ...
      const data = createUSStateData()
      const columns = createUSStateColumns()
      const testIntChart = [{ state: 'CA' }]
      
      const spec = createMapSpec(config, data, columns, false, testIntChart)
  ...
  describe('Mobile Optimizations', () => {
    it('should apply mobile-specific configurations for scatter maps', () => {
      ...
      const spec = createMapSpec(config, data, columns, true, intChart) // mobile = true
  ...
  describe('Error Handling', () => {
    it('should handle empty data gracefully', () => {
      ...
      const spec = createMapSpec(config, [], columns, false, intChart)
    ...
    it('should handle missing required fields gracefully', () => {
      ...
      }).toThrow('Both xField and yField must be provided for scatter map')
    })
