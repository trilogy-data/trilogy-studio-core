...
describe('objectToSqlExpression', () => {
  ...
  describe('single object input', () => {
    ...
    it('should handle boolean values', () => {
      expect(objectToSqlExpression({ active: true, deleted: false })).toBe('active=true AND deleted=false');
    });
  ...
  describe('edge cases', () => {
    ...
    it('should handle objects with special characters in string values', () => {
      expect(objectToSqlExpression({ query: "SELECT * FROM users" }))
        .toBe("query='SELECT * FROM users'");
    });

    it('should handle objects with string values containing JSON', () => {
      expect(objectToSqlExpression({ data: '{"name":"John"}' }))
        .toBe("data='{\"name\":\"John\"}'");
    });
