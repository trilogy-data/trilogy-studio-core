 ...
 */
const createFieldEncoding = (fieldName: string, columns: Map<string, ResultColumn>, axisOptions = {}): any => {
    if (!fieldName) return {}
 ...
 */
const createColorEncoding = (colorField: string | undefined, columns: Map<string, ResultColumn>, isMobile: boolean = false) => {
...
const createBrushParam = (intChart: Array<Partial<ChartConfig>>, config: ChartConfig) => {
    return {
        ...
        //@ts-ignore
        value: intChart.length > 0 && config.xField
    ...
    return {
        ...
        encoding: {
            ...
            color: {
                ...
                title: snakeCaseToCapitalizedWords(columns.get(config.colorField || '')?.description || config.colorField),
                            ...
                            tooltip: [
                                {
                                    field: config.xField,
                                    type: 'quantitative',
                                    title: snakeCaseToCapitalizedWords(config.xField
                                ...
                                },
                                {
                                    field: config.yField,
                                    type: 'quantitative',
                                    title: snakeCaseToCapitalizedWords(config.yField
...
export const generateVegaSpec = (
    ...
    if (config.trellisField && config.chartType === 'line') {
        spec.facet = {
            ...
            title: snakeCaseToCapitalizedWords(columns.get(config.trellisField)?.description || config.trellisField)
        }
