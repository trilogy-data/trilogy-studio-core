...
/**
 * Looks up a country based on the provided query and lookup type
 * @param countries Array of country objects in the GitHub format
 * @param query The search query (alpha-3 code, country code, or name)
 * @param lookupType The type of lookup to perform
 * @returns The matching country object or undefined if not found
 */
export function lookupCountry(
  query: string,
  lookupType: 'alpha-3' | 'country-code' | 'name',
): Country | undefined {
  // Normalize the query based on lookup type
  let normalizedQuery = query.trim()

  if (lookupType === 'name') {
    // Convert to title case (init cap) for name lookup
    normalizedQuery = normalizedQuery
      .toLowerCase()
      .split(/\s+/)
      .map((word) => word.charAt(0).toUpperCase() + word.slice(1))
      .join(' ')
  } else if (lookupType === 'alpha-3') {
    // Ensure alpha-3 is uppercase
    normalizedQuery = normalizedQuery.toUpperCase()
  }

  // Perform the lookup based on the specified type
  return CountryLookup.find((country) => {
    if (lookupType === 'name') {
      return country.name === normalizedQuery
    } else if (lookupType === 'alpha-3') {
      return country['alpha-3'] === normalizedQuery
    } else if (lookupType === 'country-code') {
      // Ensure leading zeros are preserved for comparison
      return country['country-code'] === normalizedQuery
    }
    return false
  })
}
