...
export interface QueryRecord {
  ...
  class QueryHistoryStorage {
    ...
    public async addQuery(connectionName: string, queryData: Omit<QueryRecord, 'id' | 'connectionName' | 'timestamp'>): Promise<number> {
    ...
    public async getQueriesForConnection(connectionName: string, limit: number = MAX_HISTORY_SIZE): Promise<QueryRecord[]> {
      if (!this.db) await this._initDB();
