...
const IMPORTS = {
  names: `import std.date;
import std.geography;
import std.display;


key name string; # Given name of a person at birth. Firrst name only, typically with first letter capitalized. 
...
property <id, team.id>.field_goals_pct float; # Team field goal percentage
property <id, team.id>._three_points_att int; # Team three-pointers attempted
property <id, team.id>._three_points_made int; # Team three-pointers made
property <id, team.id>.three_points_pct float; # Team three-point shot percentage
...
property <id, team.id>.two_points_pct float; # Team two-point shot percentage
...
property <id, team.id>.free_throws_pct float; # Team free throw percentage
...
# Metrics
metric three_points_att <- sum(_three_points_att); # Team three point attempts
metric three_points_made <- sum(_three_points_made); # Team three points made
auto three_points_pct <- three_points_made / three_points_att * 100; # Team three point percentage
...
datasource team_games_sr (
    ...
    team_league_alias:team_league_alias,
    team_conf_id:team_conf_id,
    team_conf_name:team_conf_name,
    team_conf_alias:team_conf_alias,
    team_division_id:team_division_id,
    team_division_name:team_division_name,
    team_division_alias:team_division_alias,
    team_logo_large:team_logo_large,
    team_logo_medium:team_logo_medium,
    team_logo_small:team_logo_small,
    ...
    three_points_att:_three_points_att,
    three_points_made:_three_points_made,
...
key division.id string;
property division.id.name string;
property division.id.alias string;

key league.id string;
property league.id.name string;
property league.id.alias string;

key conference.id string;
property conference.id.name string;
property conference.id.alias string;

key id string;
unique property id.code_ncaa int;
